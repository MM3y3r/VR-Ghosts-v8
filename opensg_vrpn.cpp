...
NodeTransitPtr ghostFactory(OSG::Time startTime){
	...
	//randome bestimmen auf welcher wand der geist gespawned wird und entsprechend ghosCT zuweißen.
	...
	for (auto &ghost : ghosts) {
			...
			{
				case (0):
				ghostCT->setRotation(Quaternion(Vec3f(0,1,0),osgDegree2Rad(0)));
				ghostCT->setTranslation(Vec3f(50,50,0));
					break;
				case (1):
				ghostCT->setRotation(Quaternion(Vec3f(1,0,0),osgDegree2Rad(90)));
				ghostCT->setTranslation(Vec3f(0,0,50));
					break;
				case (2):
				ghostCT->setRotation(Quaternion(Vec3f(0,0,1),osgDegree2Rad(180)));
				ghostCT->setTranslation(Vec3f(0,0,-50));
					break;
				case (3):
				ghostCT->setRotation(Quaternion(Vec3f(-1,0,0),osgDegree2Rad(270)));
				ghostCT->setTranslation(Vec3f(-50,0,0));
					break;
			};
	}

	//ghostCT->setRotation(Quaternion(Vec3f(1,0,0),osgDegree2Rad(120)));
...
auto head_position = Vec3f(0.f, 0.f, 40.f);	// a 1.7m Person 2m in front of the scene
...
void setupGLUT(int *argc, char *argv[])
{
	...
	glutDisplayFunc([]()
	{
		...
		double sineWave;
		sineWave = sin (10*timeSinceStart*3.14/180);
		printf ("The sine of %f degrees is %f.\n", timeSinceStart * 10, sineWave );
		...
		for (auto &ghost : ghosts) {
			...
			//If abfrage auf welcher wand gespawnt -> entsprechende translation zuweißen
			switch (ghost.id % 4)
			{
				case (0):
		...
		std::cout << "mod time: " << std::fmod(timeSinceStart, 5.0f) << '\n';
		commitChanges();
		mgr->redraw();
